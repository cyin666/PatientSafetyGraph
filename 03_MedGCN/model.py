...
class MedGCN(nn.Module):
    def __init__(self, in_dim,  out_dims, label_dims, tasks=((0,2),), dropout=0.5):
        super(MedGCN, self).__init__()
        self.encoder = nn.ModuleList( [HGCN(in_dim, out_dims[0])] )
        self.Dropout = DictDropout(p=dropout)
        self.tasks = tasks
        self.predicter = nn.ModuleDict( {str(i): nn.Linear(out_dims[-1], label_dims) for i in tasks })
    ...
    def predict(self, z):
        ...
        adj_recon={}
        for t in self.tasks:
            adj_recon[t] = [F.sigmoid(self.predicter[str(t)](z[t[0]]))]
    ...
    def forward(self, fea_mats, adj_mats):
        ...
        for key in adj_masks.keys() & adj_mats.keys():
            for i in range(len(adj_masks[key])):
                print('key: ', key, '-> i: ', i)
                print(adj_mats[key][i].shape)
                print(adj_masks[key][i].shape)
